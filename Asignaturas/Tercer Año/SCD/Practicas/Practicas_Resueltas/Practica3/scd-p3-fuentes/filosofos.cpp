...
// ----------------Explicación-------------------------------------------------------------
/*
- Se trata del mismo fichero de filosofos-interb.cpp, pero aquí se soluciona el interbloqueo, haciendo que los 
filósofos que sean pares cojan primero el izquiero y luego el derecho, y los impares al revés.
*/
...
while ( true )
{
   ...
   if (id % 2 == 0) { // Filósofos pares toman primero el tenedor izquierdo
      cout <<"Filósofo " <<id << " solicita ten. izq." <<id_ten_izq <<endl;
      MPI_Ssend( &id, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD );

      cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
      MPI_Ssend(&id, 1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD );
     } else { // Filósofos impares toman primero el tenedor derecho
      cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
      MPI_Ssend(&id, 1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD );
