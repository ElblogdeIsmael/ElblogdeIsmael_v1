...
unsigned primera_libre = 0;       // índice de la primera celda libre del buffer
...
void funcion_hebra_productora(int num_hebra) {
    for (unsigned i = 0; i < p; i++) {
        ...
        primera_libre = (primera_libre + 1) % tam_vec;  // Actualiza índice de libre
...
void funcion_hebra_consumidora(int num_hebra) {
    for (unsigned i = 0; i < c; i++) {
        ...
        acceso_exclusivo.sem_wait();       // Acceso exclusivo al buffer
        unsigned dato = buffer[primera_ocupada]; // Toma el dato del buffer
        primera_ocupada = (primera_ocupada + 1) % tam_vec;  // Actualiza índice de ocupado
        acceso_exclusivo.sem_signal();     // Libera el acceso exclusivo al buffer
