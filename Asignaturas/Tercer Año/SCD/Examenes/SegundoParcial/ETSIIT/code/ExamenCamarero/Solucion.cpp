...
const int fil4 = 8;
const int tag_sentarsef4=2, tag_levantarsef4=3;
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    ...
    if(id!=fil4){
      ...
      //si es el filósofo 4, no pide ni suelta ningún tenedor, ya que come con las manos
      cout <<"+++++++++++++++++++++++++  Filosofo ESPECIAL" <<id << " solicita SENTARSE." <<endl;
      MPI_Ssend(&peticion, 1, MPI_INT,camarero,tag_sentarsef4,MPI_COMM_WORLD); //usamos una etiqueta especial para que la reconozca el camarero

      cout <<endl<<"Filosofo ESPECIAL" <<id <<" comienza a COMER" <<endl ;
      sleep_for( milliseconds( aleatorio<1000,1800>() ) );

      cout <<"+++++++++++++++++++++++++++  Filosofo ESPECIAL" <<id << " solicita LEVANTARSE." <<endl;
      MPI_Ssend(&peticion, 1, MPI_INT,camarero,tag_levantarsef4,MPI_COMM_WORLD); //usamos una etiqueta especial para que la reconozca el camarero

      cout<<endl << "Filosofo ESPECIAL" << id << " comienza a PENSAR" << endl;
      sleep_for( milliseconds( aleatorio<1000,1800>() ) );
...
void funcion_camarero( )
{
  ...
  while ( true )
  {
    if (sentados < 4) // si hay menos de 4 filósofos sentados
    {
      int mensaje_recibido = 0;
      while (!mensaje_recibido)
      {
        MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &mensaje_recibido, &estado);
        if (mensaje_recibido)
        {
    ...
    else if (sentados == 4) // si hay 4 filósofos sentados
    {
      int mensaje_recibido = 0;
      while (!mensaje_recibido)
      {
        MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &mensaje_recibido, &estado);
        if (mensaje_recibido && (estado.MPI_TAG == tag_levantar || (estado.MPI_TAG == tag_sentarsef4 && estado.MPI_SOURCE == fil4)))
        {
          MPI_Recv(&valor, 1, MPI_INT, estado.MPI_SOURCE, estado.MPI_TAG, MPI_COMM_WORLD, &estado);
        }
      }
    }
    else // si hay más de 4 filósofos sentados (solo puede ser el filósofo 4)
    {
      MPI_Recv(&valor, 1, MPI_INT, fil4, tag_levantarsef4, MPI_COMM_WORLD, &estado);
    }

    id_filosofo = estado.MPI_SOURCE;
    tag = estado.MPI_TAG;

    if (tag == tag_sentar || tag == tag_sentarsef4)
      sentados++;
    else if (tag == tag_levantar || (tag == tag_levantarsef4))
      sentados--;
